...
func AddUIConfig(context echo.Context) error {
	...
	result, ne := helpers.DoUIConfigDatabaseAction(config, configID, username, helpers.AddAction)
	if ne != nil {
...
func AddMultipleUIConfigs(context echo.Context) error {
	...
	for _, b := range configs {
		res, ne := helpers.DoUIConfigDatabaseAction(b, b.ID, username, helpers.AddAction)
		if ne != nil {
			log.L.Errorf("%s %s", helpers.UIConfigsTag, ne.Error())
		}
...
func UpdateUIConfig(context echo.Context) error {
	...
	// call helper function
	result, ne := helpers.DoUIConfigDatabaseAction(config, configID, username, helpers.UpdateAction)
	if ne != nil {
		log.L.Errorf("%s %s", helpers.UIConfigsTag, ne.Error())
		return context.JSON(http.StatusInternalServerError, result)
	}
...
func UpdateMultipleUIConfigs(context echo.Context) error {
	...
	for id, config := range configs {
		res, ne := helpers.DoUIConfigDatabaseAction(config, id, username, helpers.UpdateAction)
		if ne != nil {
			log.L.Errorf("%s %s", helpers.UIConfigsTag, ne.Error())
		}
		results = append(results, res)
...
func DeleteUIConfig(context echo.Context) error {
	...
	// call helper function
	result, ne := helpers.DoUIConfigDatabaseAction(config, configID, username, helpers.DeleteAction)
	if ne != nil {
		log.L.Errorf("%s %s", helpers.UIConfigsTag, ne.Error())
		return context.JSON(http.StatusInternalServerError, result)
	}
